...
if __name__ == "__main__":
    T = 1000 # time steps, số bước nhảy 
    P = 12 # Số bước quá khứ
    Q = 12 # Số bước dự đoán
    K = 4 # Số head của attention
    d = 16 # Feature đầu ra của mỗi head attention
    F = 5 # Đặc trưng đầu vào
    V = 202 # Số nút
    L = 1 # Số block Statt
    data = torch.randn((T, P, V, F))
    target = torch.randn((T, Q, V))

    model = STGNN(F, K*d, L, d)